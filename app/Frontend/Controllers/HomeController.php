...
class HomeController extends Controller
{
    ...
    public function purchase(Request $request){
        if(!empty($request)){
            ...
            if(!empty($request->prodData)){
                ...
                $servername = "ns1.ams23.siteground.eu";
    ...
    public function sendmail(Request $request){
        ...
        Thank you for visiting the SleePare Showroom.<br /><br />
        ...
        SleePare';
                ...
                $conn->query($sql);
                $conn->close();
                //echo '<h1>Thank you for visiting the SleePare Showroom.</h1><br /><h2>Please check your email for details</h2>';
                $curl = curl_init();
                curl_setopt_array($curl, array(
                    CURLOPT_URL => "https://sleepare.user.com/api/public/users/search/?email=$to",
                    CURLOPT_RETURNTRANSFER => true,
                    ...
                    {	
                        ...
                        if($response === ""){
                            ...
                            if ($err) {
                                ...
                                $prodsText = '';
                                $myUserKey	= $myResponse->key;
                                $dataArraySub = array('name' => 'showroom_email_coupon', 'client' => $myResponse->id, 'data' => array('emailfrom' => $emailFrom, 'email' => $to, 'customer_name' => $request->custname, 'keyword' => $request->keyword, 'phone' => $request->custphone, 'comments' => $request->custcom, 'products' => $productsData, 'visit_date' => date('Y-m-d'), 'visit_time' => date('H:i:s'), 'want_to_purcahse_on' => $purchaseDate));
                                $dataArraySub = json_encode($dataArraySub);
                                $curl = curl_init();
                                curl_setopt_array($curl, array(
                                    CURLOPT_URL => "https://sleepare.user.com/api/public/events/",
